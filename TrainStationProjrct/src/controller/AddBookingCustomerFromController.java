...
public class AddBookingCustomerFromController {
    ...
    public void btnBookingOnAction() {
        BookingCustomer c = new BookingCustomer(
                txtCusId.getText(), txtCusName.getText(), txtCusAddress.getText(), txtCusContact.getText(), cmbCusFrom.getValue(), cmbCusTo.getValue(), txtTrainTime.getText(), cmbCusTrain.getValue(), cmbCusSeatNo.getValue(), cmbCusClass.getValue(), txtCusPrice.getText(), txtCusBookDate.getText());
        try {
            if (CrudUtil.executeUpdate("INSERT INTO booking VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", c.getId(), c.getName(), c.getAddress(), c.getContact(),
                    c.getTrainFrom(), c.getTrainTo(), c.getTime(), c.getTrain(), c.getSeatNo(), c.getTrainClass(), c.getPrice(),c.getDate())) {
